...
export function buildModelPool() {
  ...
  try {
    // v8.2 修复：检查文件大小，防止过大文件导致阻塞
    let configRaw;
    try {
      const stats = fs.statSync(OPENCLAW_CONFIG);
      if (stats.size > 5 * 1024 * 1024) { // 5MB 限制
        console.warn(`⚠️ openclaw.json 过大 (${(stats.size / 1024 / 1024).toFixed(2)}MB > 5MB)，使用简化配置`);
        return buildSimplifiedPool();
      }
      configRaw = fs.readFileSync(OPENCLAW_CONFIG, 'utf-8');
    } catch (statError) {
      // 文件不存在或无法访问，返回空池
      console.warn(`⚠️ 无法读取 openclaw.json: ${statError.message}，使用空模型池`);
      return pool;
    }
...
/**
 * v8.2 新增：构建简化模型池（当 openclaw.json 过大时使用）
 * 仅从环境变量或部分配置加载，不读取完整配置文件
 * 
 * 设计原则：
 * 1. 不硬编码任何特定模型 ID（适配第三方环境）
 * 2. 优先从环境变量读取
 * 3. 尝试从 openclaw.json 读取 fallbacks（只读取必要部分）
 * 4. 如果都无法读取，返回空池（由调用方决定如何处理）
 */
function buildSimplifiedPool() {
  console.log('🔧 使用简化模型池（openclaw.json 过大）');
  ...
  // 1. 尝试从环境变量获取模型信息
  const modelsEnv = process.env.OPENCLAW_MODELS;
  if (modelsEnv) {
    try {
      const models = JSON.parse(modelsEnv);
      for (const model of models) {
        ...
        pool.all.push(entry);
        pool.byTier.standard.push(entry);
      }
      console.log(`✓ 从环境变量加载 ${pool.all.length} 个模型`);
      if (pool.all.length > 0) {
        return pool;
      }
    } catch (e) {
      console.warn(`⚠️ OPENCLAW_MODELS 解析失败：${e.message}`);
    }
  }
  
  // 2. 尝试从 openclaw.json 读取 fallbacks（只读取必要部分）
  try {
    const configRaw = fs.readFileSync(OPENCLAW_CONFIG, 'utf-8');
    // 使用更宽松的解析方式，只提取 fallbacks
    const fallbackMatch = configRaw.match(/"fallbacks"\s*:\s*\[([^\]]+)\]/);
    if (fallbackMatch) {
      const fallbacksRaw = fallbackMatch[1];
      // 提取模型 ID（简单的字符串提取）
      const modelIds = fallbacksRaw.match(/"([^"]+)"/g)?.map(s => s.slice(1, -1)) || [];
      
      if (modelIds.length > 0) {
        for (const modelId of modelIds) {
          const entry = {
            id: modelId,
            tier: inferTier(modelId, { input: 0, output: 0 }),
            traits: inferTraits(modelId, '', modelId.split('/')[0])
          };
          pool.all.push(entry);
          pool.byTier[entry.tier]?.push(entry);
        }
        console.log(`✓ 从 fallbacks 加载 ${pool.all.length} 个模型`);
        return pool;
      }
    }
  } catch (e) {
    console.warn(`⚠️ openclaw.json fallbacks 读取失败：${e.message}`);
  }
  
  // 3. 尝试从 openclaw.json 读取 providers 中的第一个可用模型
  try {
    const configRaw = fs.readFileSync(OPENCLAW_CONFIG, 'utf-8');
    // 提取第一个 provider 的第一个模型
    const providerMatch = configRaw.match(/"providers"\s*:\s*\{\s*"([^"]+)"\s*:\s*\{[^}]*"models"\s*:\s*\[\s*\{\s*"id"\s*:\s*"([^"]+)"/);
    if (providerMatch) {
      const providerName = providerMatch[1];
      const modelId = providerMatch[2];
      const fullId = `${providerName}/${modelId}`;
      
      const entry = {
        id: fullId,
        tier: inferTier(fullId, { input: 0, output: 0 }),
        traits: inferTraits(modelId, '', providerName)
      };
      pool.all.push(entry);
      pool.byTier[entry.tier]?.push(entry);
      console.log(`✓ 从 providers 加载 1 个模型：${fullId}`);
      return pool;
    }
  } catch (e) {
    console.warn(`⚠️ openclaw.json providers 读取失败：${e.message}`);
  }
  
  // 4. 如果所有方法都失败，返回空池（不硬编码任何模型）
  console.warn('⚠️ 无法获取任何可用模型，返回空模型池');
  console.warn('💡 建议：检查 openclaw.json 配置或设置 OPENCLAW_MODELS 环境变量');
