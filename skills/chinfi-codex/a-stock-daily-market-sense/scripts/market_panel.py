...
def build_low_position_volume_anomaly_samples(
    ...
    # Spike trigger: low position + volume ratio + pct_chg, all on the same day.
    ...
    for ts_code, sub in grouped:
        ...
        candidates.append({
            ...
            "trigger_low_track": "A" if bool(trigger_row.get("is_low_A")) else "B",
    ...
    # Sort: starter first, then sustain, then quiet, then undetermined.
    ...
    return {
        ...
        "filter_criteria": {
            ...
            "sample_limit": sample_limit,
            "scenario_priority": "starter > sustain > quiet > undetermined",
        },
        ...
        "interpretation_hints": [
            ...
            "sustain (持续换手型): trigger day in past 7 days, every later day stayed at > sustain_ratio_min of the trigger amount, price holding above trigger open. 资金在换手吃筹.",
            "quiet (缩量企稳型): trigger day in past 7 days, recent 3 days median volume <= quiet_ratio_max of the trigger amount, price holding above 0.95x trigger close. 放量拉升后缩量横盘.",
...
def build_panel(args: argparse.Namespace) -> Dict[str, Any]:
    ...
    daily = fetch_by_trade_dates(pro, "daily", trade_dates, DEFAULT_DAILY_FIELDS, args.sleep)
    if daily.empty:
    ...
    return {
        ...
        **candidates,
        ...
        "notes": [
            ...
            "limit_up_approx_count and limit_down_approx_count are daily pct_chg threshold approximations. Prefer limit_stats official counts when available.",
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    panel.add_argument("--skip-limit", action="store_true", help="Skip limit_list_d retrieval.")
    ...
    panel.add_argument("--low-lookback-days", type=int, default=7,
                       help="Low-position pool: how many trading days to look back for the trigger day (default 7).")
