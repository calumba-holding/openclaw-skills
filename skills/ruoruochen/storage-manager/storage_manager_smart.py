...
class SmartLocationMatcher:
    ...
    def get_matching_location(self, new_location: str) -> str:
        ...
        if not self.existing_options:
            print(f"[智能匹配] 无现有选项，创建: '{new_location}'")
            return new_location
...
def main():
    ...
    try:
        if args.command == "add":
            ...
            print(f"✅ 物品入库成功（智能位置匹配）")
            print(f"   物品: {args.item}")
            ...
            print(f"✅ 位置更新成功（智能位置匹配）")
            print(f"   物品: {args.item}")
            print(f"   新位置: {args.new_location}")
