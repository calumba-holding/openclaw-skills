#!/usr/bin/env bash
# Security Roadmap — content tool
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

DATA_DIR="${HOME}/.local/share/security-roadmap"
mkdir -p "$DATA_DIR"

_log() { echo "$(date '+%m-%d %H:%M') $1: $2" >> "$DATA_DIR/history.log"; }
_version() { echo "security-roadmap v2.0.0"; }

_help() {
    echo "Security Roadmap v2.0.0 — content toolkit"
    echo ""
    echo "Usage: security-roadmap <command> [args]"
    echo ""
    echo "Commands:"
    echo "  draft              Draft"
    echo "  edit               Edit"
...
_export() {
    ...
    case "$fmt" in
        ...
        csv)
            ...
            ;;
        txt)
            echo "=== Security Roadmap Export ===" > "$out"
            for f in "$DATA_DIR"/*.log; do
                [ -f "$f" ] || continue
...
_search() {
    local term="${1:?Usage: security-roadmap search <term>}"
...
case "${1:-help}" in
    draft)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/draft.log" 2>/dev/null || echo "  No entries yet. Use: security-roadmap draft <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/draft.log"
            local total=$(wc -l < "$DATA_DIR/draft.log")
            echo "  [Security Roadmap] draft: $input"
    ...
    edit)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/edit.log" 2>/dev/null || echo "  No entries yet. Use: security-roadmap edit <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/edit.log"
            local total=$(wc -l < "$DATA_DIR/edit.log")
            echo "  [Security Roadmap] edit: $input"
    ...
    optimize)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/optimize.log" 2>/dev/null || echo "  No entries yet. Use: security-roadmap optimize <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/optimize.log"
            local total=$(wc -l < "$DATA_DIR/optimize.log")
            echo "  [Security Roadmap] optimize: $input"
    ...
    schedule)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/schedule.log" 2>/dev/null || echo "  No entries yet. Use: security-roadmap schedule <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/schedule.log"
            local total=$(wc -l < "$DATA_DIR/schedule.log")
            echo "  [Security Roadmap] schedule: $input"
    ...
    hashtags)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/hashtags.log" 2>/dev/null || echo "  No entries yet. Use: security-roadmap hashtags <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/hashtags.log"
            local total=$(wc -l < "$DATA_DIR/hashtags.log")
            echo "  [Security Roadmap] hashtags: $input"
    ...
    hooks)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/hooks.log" 2>/dev/null || echo "  No entries yet. Use: security-roadmap hooks <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/hooks.log"
            local total=$(wc -l < "$DATA_DIR/hooks.log")
            echo "  [Security Roadmap] hooks: $input"
    ...
    cta)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/cta.log" 2>/dev/null || echo "  No entries yet. Use: security-roadmap cta <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/cta.log"
            local total=$(wc -l < "$DATA_DIR/cta.log")
            echo "  [Security Roadmap] cta: $input"
    ...
    rewrite)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/rewrite.log" 2>/dev/null || echo "  No entries yet. Use: security-roadmap rewrite <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/rewrite.log"
            local total=$(wc -l < "$DATA_DIR/rewrite.log")
            echo "  [Security Roadmap] rewrite: $input"
    ...
    translate)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/translate.log" 2>/dev/null || echo "  No entries yet. Use: security-roadmap translate <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/translate.log"
            local total=$(wc -l < "$DATA_DIR/translate.log")
            echo "  [Security Roadmap] translate: $input"
    ...
    tone)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/tone.log" 2>/dev/null || echo "  No entries yet. Use: security-roadmap tone <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/tone.log"
            local total=$(wc -l < "$DATA_DIR/tone.log")
            echo "  [Security Roadmap] tone: $input"
    ...
    headline)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/headline.log" 2>/dev/null || echo "  No entries yet. Use: security-roadmap headline <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/headline.log"
            local total=$(wc -l < "$DATA_DIR/headline.log")
            echo "  [Security Roadmap] headline: $input"
    ...
    outline)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/outline.log" 2>/dev/null || echo "  No entries yet. Use: security-roadmap outline <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/outline.log"
            local total=$(wc -l < "$DATA_DIR/outline.log")
            echo "  [Security Roadmap] outline: $input"
    ...
    *)
        echo "Unknown: $1 — run 'security-roadmap help'"
        exit 1
        ;;
esac