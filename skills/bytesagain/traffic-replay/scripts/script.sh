#!/usr/bin/env bash
# Traffic Replay — utility tool
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

DATA_DIR="${HOME}/.local/share/traffic-replay"
mkdir -p "$DATA_DIR"

_log() { echo "$(date '+%m-%d %H:%M') $1: $2" >> "$DATA_DIR/history.log"; }
_version() { echo "traffic-replay v2.0.0"; }

_help() {
    echo "Traffic Replay v2.0.0 — utility toolkit"
    echo ""
    echo "Usage: traffic-replay <command> [args]"
    echo ""
    echo "Commands:"
    echo "  run                Run"
    echo "  check              Check"
...
_export() {
    ...
    case "$fmt" in
        ...
        csv)
            ...
            ;;
        txt)
            echo "=== Traffic Replay Export ===" > "$out"
            for f in "$DATA_DIR"/*.log; do
                [ -f "$f" ] || continue
...
_search() {
    local term="${1:?Usage: traffic-replay search <term>}"
...
case "${1:-help}" in
    run)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/run.log" 2>/dev/null || echo "  No entries yet. Use: traffic-replay run <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/run.log"
            local total=$(wc -l < "$DATA_DIR/run.log")
            echo "  [Traffic Replay] run: $input"
    ...
    check)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/check.log" 2>/dev/null || echo "  No entries yet. Use: traffic-replay check <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/check.log"
            local total=$(wc -l < "$DATA_DIR/check.log")
            echo "  [Traffic Replay] check: $input"
    ...
    convert)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/convert.log" 2>/dev/null || echo "  No entries yet. Use: traffic-replay convert <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/convert.log"
            local total=$(wc -l < "$DATA_DIR/convert.log")
            echo "  [Traffic Replay] convert: $input"
    ...
    analyze)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/analyze.log" 2>/dev/null || echo "  No entries yet. Use: traffic-replay analyze <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/analyze.log"
            local total=$(wc -l < "$DATA_DIR/analyze.log")
            echo "  [Traffic Replay] analyze: $input"
    ...
    generate)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/generate.log" 2>/dev/null || echo "  No entries yet. Use: traffic-replay generate <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/generate.log"
            local total=$(wc -l < "$DATA_DIR/generate.log")
            echo "  [Traffic Replay] generate: $input"
    ...
    preview)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/preview.log" 2>/dev/null || echo "  No entries yet. Use: traffic-replay preview <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/preview.log"
            local total=$(wc -l < "$DATA_DIR/preview.log")
            echo "  [Traffic Replay] preview: $input"
    ...
    batch)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/batch.log" 2>/dev/null || echo "  No entries yet. Use: traffic-replay batch <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/batch.log"
            local total=$(wc -l < "$DATA_DIR/batch.log")
            echo "  [Traffic Replay] batch: $input"
    ...
    compare)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/compare.log" 2>/dev/null || echo "  No entries yet. Use: traffic-replay compare <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/compare.log"
            local total=$(wc -l < "$DATA_DIR/compare.log")
            echo "  [Traffic Replay] compare: $input"
    ...
    export)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/export.log" 2>/dev/null || echo "  No entries yet. Use: traffic-replay export <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/export.log"
            local total=$(wc -l < "$DATA_DIR/export.log")
            echo "  [Traffic Replay] export: $input"
    ...
    config)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/config.log" 2>/dev/null || echo "  No entries yet. Use: traffic-replay config <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/config.log"
            local total=$(wc -l < "$DATA_DIR/config.log")
            echo "  [Traffic Replay] config: $input"
    ...
    status)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/status.log" 2>/dev/null || echo "  No entries yet. Use: traffic-replay status <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/status.log"
            local total=$(wc -l < "$DATA_DIR/status.log")
            echo "  [Traffic Replay] status: $input"
    ...
    report)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/report.log" 2>/dev/null || echo "  No entries yet. Use: traffic-replay report <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/report.log"
            local total=$(wc -l < "$DATA_DIR/report.log")
            echo "  [Traffic Replay] report: $input"
    ...
    *)
        echo "Unknown: $1 — run 'traffic-replay help'"
        exit 1
        ;;
esac