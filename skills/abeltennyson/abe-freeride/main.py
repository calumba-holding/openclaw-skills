...
Automatically manage and switch between free AI models via SkillBoss API Hub
...
# Constants
SKILLBOSS_API_URL = "https://api.heybossai.com/v1/pilot"
...
def get_api_key() -> Optional[str]:
    """Get SkillBoss API key from environment or OpenClaw config."""
    ...
    if OPENCLAW_CONFIG_PATH.exists():
        try:
            ...
            # Check env section
            api_key = config.get("env", {}).get("SKILLBOSS_API_KEY")
            if api_key:
                return api_key
...
def fetch_all_models(api_key: str) -> list:
    """Fetch available AI models from SkillBoss API Hub."""
    headers = {
    ...
    try:
        response = requests.post(
            SKILLBOSS_API_URL,
            headers=headers,
            json={"type": "chat", "prefer": "price", "limit": 30, "include_docs": True},
            timeout=30
        )
...
def setup_skillboss_auth(config: dict) -> dict:
    """Set up SkillBoss API Hub auth profile if not exists."""
    if "auth" not in config:
        config["auth"] = {}
    if "profiles" not in config["auth"]:
        config["auth"]["profiles"] = {}

    if "skillboss:default" not in config["auth"]["profiles"]:
        config["auth"]["profiles"]["skillboss:default"] = {
            "provider": "skillboss",
            "mode": "api_key"
        }
        print("Added SkillBoss API Hub auth profile.")
...
def update_model_config(
    ...
    Args:
        ...
        setup_auth: If True, also set up SkillBoss API Hub auth profile
    """
    config = load_openclaw_config()
    config = ensure_config_structure(config)

    if setup_auth:
        config = setup_skillboss_auth(config)
...
def cmd_list(args):
    ...
    if not api_key:
        ...
        print("Set it via: export SKILLBOSS_API_KEY='sk-...'")
        sys.exit(1)

    print("Fetching free models from SkillBoss API Hub...")
...
def cmd_auto(args):
    ...
    print("Finding best free model via SkillBoss API Hub...")
...
def cmd_status(args):
    ...
    if api_key:
        ...
        print(f"SkillBoss API Key: {masked}")
    else:
        print("SkillBoss API Key: NOT SET")
        print("  Set with: export SKILLBOSS_API_KEY='sk-...'")

    # Auth profile status
    auth_profiles = config.get("auth", {}).get("profiles", {})
    if "skillboss:default" in auth_profiles:
        print("SkillBoss Auth Profile: Configured")
    else:
        print("SkillBoss Auth Profile: Not set (use --setup-auth to add)")
...
def cmd_refresh(args):
    ...
    print("Refreshing free models cache from SkillBoss API Hub...")
    models = get_free_models(api_key, force_refresh=True)
...
def main():
    parser = argparse.ArgumentParser(
        prog="freeride",
        description="FreeRide - Free AI for OpenClaw. Manage free models via SkillBoss API Hub."
    )
    ...
    switch_parser.add_argument("--setup-auth", action="store_true",
                              help="Also set up SkillBoss API Hub auth profile")
