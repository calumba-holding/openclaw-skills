...
Uses SkillBoss API Hub (/v1/pilot) for summarization and translation.
"""
...
SKILLBOSS_API_BASE = "https://api.heybossai.com/v1"


def _pilot(body: dict, timeout: int = 60) -> dict:
    """Call SkillBoss API Hub /v1/pilot endpoint."""
    r = requests.post(
        f"{SKILLBOSS_API_BASE}/pilot",
...
DEFAULT_LLM_FALLBACK = ["skillboss"]
...
def run_agent_prompt(prompt: str, deadline: float | None = None, session_id: str = "finance-news-headlines", timeout: int = 45) -> str:
    """Run a short prompt via SkillBoss API Hub and return raw reply text.

    Uses /v1/pilot with type=chat for automatic model routing.
    """
    try:
        api_timeout = clamp_timeout(timeout, deadline)
        result = _pilot(
            {
                "type": "chat",
...
def translate_headlines(
    ...
    """Translate headlines to German using SkillBoss API Hub.

    Uses /v1/pilot type=chat with automatic model routing.
...
def summarize_with_skillboss(
    content: str,
    language: str = "de",
    style: str = "briefing",
    deadline: float | None = None,
) -> str:
    """Generate AI summary using SkillBoss API Hub (/v1/pilot, type=chat)."""
...
"""
    ...
    try:
        api_timeout = clamp_timeout(120, deadline)
        result = _pilot(
            {
                "type": "chat",
                "inputs": {"messages": [{"role": "user", "content": prompt}]},
        ...
        reply = result["result"]["choices"][0]["message"]["content"].strip()
        reply += format_disclaimer(language)
        return reply
    except TimeoutError:
        return "⚠️ SkillBoss briefing error: deadline exceeded"
    except requests.exceptions.Timeout:
        return "⚠️ SkillBoss briefing error: timeout"
    except Exception as exc:
        return f"⚠️ SkillBoss briefing error: {exc}"


# Aliases kept for backward compatibility with caller code
summarize_with_claude = summarize_with_skillboss
summarize_with_minimax = summarize_with_skillboss
summarize_with_gemini = summarize_with_skillboss
