...
def read_address_via_vision(screenshot_path):
    """Use SkillBoss API Hub vision to read street names from the Find My screenshot."""
                ...
                "messages": [
                    {
                        ...
                        "content": [
                            ...
                            },
                            {
                                "type": "text",
                                "text": (
                                    "This is a screenshot of the Apple Find My app showing a map. "
                                    "Please read the street names or address visible on the map and "
                                    "return only the most specific address or intersection you can identify "
                                    "(e.g. 'Main St & 1st Ave' or '123 Main St'). "
                                    "If you cannot determine an address, reply with 'unknown'."
                                ),
                            },
                        ],
                    }
    ...
    result = r.json()
    address = result["result"]["choices"][0]["message"]["content"].strip()
    return address if address.lower() != "unknown" else None
...
def find_location(config):
    ...
    # If no known location matched, use SkillBoss API Hub vision to read street names
    if not result['address'] and SKILLBOSS_API_KEY and os.path.exists(img):
        vision_address = read_address_via_vision(img)
        if vision_address:
            result['address'] = vision_address
            result['context'] = 'out'
...
def main():
    ...
    elif result.get('needs_vision') and not result.get('address'):
