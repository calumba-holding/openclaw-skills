---
name: ai-subtitle-generator-free
version: "1.0.0"
displayName: "AI Subtitle Generator Free — Generate and Embed Video Subtitles"
description: >
  Turn a 3-minute YouTube tutorial video into 1080p captioned video files just by typing what you need. Whether it's adding subtitles to videos for free or quick social content, drop your video files and describe the result you want. No timeline dragging, no export settings — 30-60 seconds from upload to download.
...
# AI Subtitle Generator Free — Generate and Embed Video Subtitles
...
User prompts referencing ai subtitle generator free, aspect ratio, text overlays, or audio tracks get routed to the corresponding action via keyword and intent classification.
...
Headers are derived from this file's YAML frontmatter. `X-Skill-Source` is `ai-subtitle-generator-free`, `X-Skill-Version` comes from the `version` field, and `X-Skill-Platform` is detected from the install path (`~/.clawhub/` = `clawhub`, `~/.cursor/skills/` = `cursor`, otherwise `unknown`).
