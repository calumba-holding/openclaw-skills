...
// 92 RSS feeds from Hacker News Popularity Contest 2025 (curated by Karpathy) + 3 Chinese tech media
const RSS_FEEDS: Array<{ name: string; xmlUrl: string; htmlUrl: string }> = [
  ...
  { name: "InfoQ中文", xmlUrl: "https://www.infoq.cn/feed", htmlUrl: "https://www.infoq.cn" },
];
...
// ============================================================================
// AI Providers (OpenAI-compatible primary + Gemini fallback)
// ============================================================================
  ...
  return {
    async call(prompt: string): Promise<string> {
      if (state.openaiApiKey) {
        try {
          return await callOpenAICompatible(prompt, state.openaiApiKey, state.openaiApiBase, state.openaiModel);
        } catch (error) {
          if (state.geminiApiKey) {
            if (!state.fallbackLogged) {
              const reason = error instanceof Error ? error.message : String(error);
              console.warn(`[digest] OpenAI-compatible failed (${state.openaiApiBase}, model=${state.openaiModel}), switching to Gemini fallback. Reason: ${reason}`);
      ...
      throw new Error('No AI API key configured. Set OPENAI_API_KEY and/or GEMINI_API_KEY.');
    },
  };
}
...
function generateDigestReport(articles: ScoredArticle[], highlights: string, stats: {
  ...
  report += `> 来自 Karpathy 推荐的 ${stats.totalFeeds} 个顶级技术博客及国内科技媒体，AI 精选 Top ${articles.length}\n\n`;
  ...
  report += `*基于 [Hacker News Popularity Contest 2025](https://refactoringenglish.com/tools/hn-popularity/) RSS 源列表（92 个海外技术博客 + 3 个国内科技媒体），由 [Andrej Karpathy](https://x.com/karpathy) 推荐*\n`;
  report += `*由「饭团AI」制作，欢迎关注同名微信公众号获取更多 AI 实用技巧 💡*\n`;
...
function printUsage(): never {
  console.log(`AI Daily Digest - AI-powered RSS digest from 95 top tech blogs (92 HN + 3 Chinese)
...
Environment:
  OPENAI_API_KEY   Primary API key for OpenAI-compatible services (DeepSeek, OpenAI, etc.)
  ...
  GEMINI_API_KEY   Optional fallback key for Google Gemini. Get one at https://aistudio.google.com/apikey
...
async function main(): Promise<void> {
  ...
  if (!geminiApiKey && !openaiApiKey) {
    console.error('[digest] Error: Missing API key. Set OPENAI_API_KEY and/or GEMINI_API_KEY.');
    console.error('[digest] OpenAI-compatible: set OPENAI_API_KEY (and optionally OPENAI_API_BASE for DeepSeek, etc.)');
    console.error('[digest] Gemini fallback: https://aistudio.google.com/apikey');
    process.exit(1);
  }
  ...
  console.log(`[digest] AI provider: ${openaiApiKey ? 'OpenAI-compatible (primary)' : 'Gemini (primary)'}`);
  if (geminiApiKey) {
    console.log(`[digest] Fallback: Gemini`);
  }
  console.log('');
