...
DATA_DIR="${HOME}/.local/share/tar"
mkdir -p "$DATA_DIR"

_log() { echo "$(date '+%m-%d %H:%M') $1: $2" >> "$DATA_DIR/history.log"; }

_version() { echo "tar v2.0.0"; }

_help() {
    echo "Tar v2.0.0 — sysops toolkit"
    echo ""
    echo "Usage: tar <command> [args]"
    echo ""
    echo "Commands:"
    echo "  scan               Scan"
...
case "${1:-help}" in
    scan)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/scan.log" 2>/dev/null || echo "  No entries yet. Use: tar scan <input>"
        else
            local input="$*"
    ...
    monitor)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/monitor.log" 2>/dev/null || echo "  No entries yet. Use: tar monitor <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/monitor.log"
            local total=$(wc -l < "$DATA_DIR/monitor.log")
            echo "  [Tar] monitor: $input"
    ...
    report)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/report.log" 2>/dev/null || echo "  No entries yet. Use: tar report <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/report.log"
            local total=$(wc -l < "$DATA_DIR/report.log")
            echo "  [Tar] report: $input"
    ...
    alert)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/alert.log" 2>/dev/null || echo "  No entries yet. Use: tar alert <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/alert.log"
            local total=$(wc -l < "$DATA_DIR/alert.log")
            echo "  [Tar] alert: $input"
    ...
    top)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/top.log" 2>/dev/null || echo "  No entries yet. Use: tar top <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
    ...
    usage)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/usage.log" 2>/dev/null || echo "  No entries yet. Use: tar usage <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/usage.log"
            local total=$(wc -l < "$DATA_DIR/usage.log")
            echo "  [Tar] usage: $input"
    ...
    check)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/check.log" 2>/dev/null || echo "  No entries yet. Use: tar check <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/check.log"
            local total=$(wc -l < "$DATA_DIR/check.log")
            echo "  [Tar] check: $input"
    ...
    fix)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/fix.log" 2>/dev/null || echo "  No entries yet. Use: tar fix <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
    ...
    cleanup)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/cleanup.log" 2>/dev/null || echo "  No entries yet. Use: tar cleanup <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/cleanup.log"
            local total=$(wc -l < "$DATA_DIR/cleanup.log")
            echo "  [Tar] cleanup: $input"
    ...
    backup)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/backup.log" 2>/dev/null || echo "  No entries yet. Use: tar backup <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/backup.log"
            local total=$(wc -l < "$DATA_DIR/backup.log")
            echo "  [Tar] backup: $input"
    ...
    restore)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/restore.log" 2>/dev/null || echo "  No entries yet. Use: tar restore <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/restore.log"
            local total=$(wc -l < "$DATA_DIR/restore.log")
            echo "  [Tar] restore: $input"
    ...
    log)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/log.log" 2>/dev/null || echo "  No entries yet. Use: tar log <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
    ...
    benchmark)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/benchmark.log" 2>/dev/null || echo "  No entries yet. Use: tar benchmark <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/benchmark.log"
            local total=$(wc -l < "$DATA_DIR/benchmark.log")
            echo "  [Tar] benchmark: $input"
    ...
    compare)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/compare.log" 2>/dev/null || echo "  No entries yet. Use: tar compare <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/compare.log"
            local total=$(wc -l < "$DATA_DIR/compare.log")
            echo "  [Tar] compare: $input"
    ...
    *)
        ...
        echo "Run 'tar help' for available commands."
        exit 1
        ;;
esac