#!/usr/bin/env bash
# shop-device.sh — 분야의 기기별 트렌드 (pc / mo).
#
# Usage:
#   shop-device.sh --start YYYY-MM-DD --end YYYY-MM-DD --time-unit (date|week|month) \
#                  --category 50000000
#
# No keywords; the API returns the time series split by device. Output: JSONL.
set -euo pipefail
...
resp=$(datalab_post "shopping/category/device" "$body")
