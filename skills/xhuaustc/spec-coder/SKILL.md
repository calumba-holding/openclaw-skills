...
|-------|--------|------|------|
| 0. Codebase Scan | context summary | — | Phase 1 |
| 1. Clarify & Scope | `requirements.md` | Gate 1 (req.) | Phase 2a |
| 2a. Technical Design | `design.md` | — | Phase 2b |
| 2b. Design Preview | `design-preview/` | Gate 2 (design) | Phase 2c |
| 2c. Task Breakdown | `tasks.md` | — | Phase 2d |
| 2d. Feature Specs | `spec_xxx.md` | Gate 3 (plan) | Phase 3 |
| 3. Generate | code + tests | Gate 4 (code) | Phase 4 |
| 4. Verify | test results | — | Phase 5 or done |
| 5. Evolve | changes/ | scoped gate | → Phase 1–4 |
...
For existing codebases, run before Phase 1: read dependency files to detect tech stack, list directory tree, identify existing interfaces/models/conventions, and summarize findings as context for all subsequent phases.
...
2. **Structured Requirements** — Produce `specs/requirements.md` (see [template](references/templates.md#requirementsmd-template)): Background & Objectives, User Roles & Use Cases, Functional Requirements (FR-001...), Non-Functional Requirements, Out of Scope, Existing Architecture (if Phase 0 ran).
...
For existing codebases: include "Existing Architecture" section (what's new vs. modified).

### 2b: Design Preview → `specs/design-preview/`

Self-contained HTML prototype visualizing architecture, module layout, UI screens or API flows, and data models. For non-UI projects: Architecture Diagram Preview only (module relationships + data flow).
...
Task table (see [template](references/templates.md#tasksmd-template)): Task ID, Name, Module, Dependencies, Related Spec, Acceptance Criteria, AI-Automatable. Target granularity: 30–90 min per task.
...
2. For each task in `tasks.md`, take its linked `spec_xxx.md` as input.
3. Output per task:
...
4. Tag generated code with task IDs in commit messages: `feat(TASK-001): ...`
5. Add spec traceability comments at module/class level: `// SPEC: spec_auth.md | TASK-002` — enables reverse lookup from code to spec.
...
1. Execute test suite (e.g., `pytest`, `npm test`, `go test`)
2. Run linter / type checker if available
3. Compare test results against spec Test Points — map each TP-ID to pass/fail
...
### Step 2: Work Within the Change Directory
...
### Step 3: Cross-Feature Impact Analysis
...
1. Generate `delta.md` — merge instructions listing every trunk file/section to update.
2. Apply delta to trunk specs (`specs/*.md`): insert ADDED content, replace MODIFIED sections, remove REMOVED items.
3. Add changelog entries to every modified trunk file.
4. Update `specs/index.md` with the change reference.
5. Move change directory to `specs/changes/archive/`.
