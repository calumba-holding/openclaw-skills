...
def should_use_fake_ip():
    use_fake_ip = os.environ.get('USE_FAKE_IP', 'true').lower()
    return use_fake_ip != 'false'

def fake_ip():
    return f"{223}.{random.randint(64, 117)}.{random.randint(0, 255)}.{random.randint(0, 255)}"

def encrypt_login_data(plain):
...
def run(user, password, step_min, step_max, use_fake_ip=None):
    device_id = str(uuid.uuid4())
    use_fake = should_use_fake_ip() if use_fake_ip is None else use_fake_ip
    fake_addr = fake_ip() if use_fake else None
    ...
    if fake_addr:
        login_headers["X-Forwarded-For"] = fake_addr
        login_headers["X-Real-IP"] = fake_addr
    ...
    for attempt in range(2):
        ...
        if r1.status_code == 429:
            ...
            continue
        return {"success": False, "user": user, "step": "?", "error": f"Login failed ({r1.status_code})"}

    if r1 is None or r1.status_code != 303:
        return {"success": False, "user": user, "step": "?", "error": "No redirect received"}
    ...
    try:
        ...
        if result is None or len(result) == 0:
            return {"success": False, "user": user, "step": "?", "error": f"Cannot extract access code"}
        code = result[0]
    except Exception as e:
        return {"success": False, "user": user, "step": "?", "error": f"Location parse error: {e}"}
    ...
    try:
        ...
        if r2.status_code != 200:
            return {"success": False, "user": user, "step": "?", "error": f"Get login_token failed ({r2.status_code})"}
        info = r2.json()
        if "token_info" not in info:
            return {"success": False, "user": user, "step": "?", "error": "Response missing token_info"}
        login_token = info["token_info"]["login_token"]
        userid = info["token_info"]["user_id"]
    except Exception as e:
        return {"success": False, "user": user, "step": "?", "error": f"Get login_token error: {e}"}

    # 3. Get app_token
    url3 = f"https://account-cn.huami.com/v1/client/app_tokens?app_name=com.xiaomi.hm.health&dn=api-user.huami.com%2Capi-mifit.huami.com%2Capp-analytics.huami.com&login_token={login_token}"
    headers3 = {'User-Agent': 'MiFit/5.3.0 (iPhone; iOS 14.7.1; Scale/3.00)'}
    try:
        r3 = requests.get(url3, headers=headers3, timeout=15)
        app_token = r3.json()['token_info']['app_token']
    except:
        return {"success": False, "user": user, "step": "?", "error": "Get app_token failed"}
    ...
    date_matches = finddate.findall(data_json)
    step_matches = findstep.findall(data_json)

    if not date_matches or not step_matches:
        return {"success": False, "user": user, "step": "?", "error": "Template parse failed"}

    data_json = re.sub(date_matches[0], today, str(data_json))
    data_json = re.sub(step_matches[0], step, str(data_json))
    ...
    try:
        ...
        msg = response.get('message', 'success' if success else 'failed')
        return {
            "success": success,
        ...
        return {"success": False, "user": user, "step": step, "error": f"Step submit error: {e}"}
