---
name: highlight-editor-app
version: "1.0.0"
displayName: "Highlight Editor App — Extract and Export Video Highlights"
description: >
  Turn a 2-hour sports game recording into 1080p highlight reel clips just by typing what you need. Whether it's generating short highlight reels from long video recordings or quick social content, drop your raw video footage and describe the result you want. No timeline dragging, no export settings — 1-2 minutes from upload to download.
...
# Highlight Editor App — Extract and Export Video Highlights
...
Worth noting: trimming your source video to the relevant section before uploading speeds up highlight detection.

## Matching Input to Actions

User prompts referencing highlight editor app, aspect ratio, text overlays, or audio tracks get routed to the corresponding action via keyword and intent classification.
...
Headers are derived from this file's YAML frontmatter. `X-Skill-Source` is `highlight-editor-app`, `X-Skill-Version` comes from the `version` field, and `X-Skill-Platform` is detected from the install path (`~/.clawhub/` = `clawhub`, `~/.cursor/skills/` = `cursor`, otherwise `unknown`).
