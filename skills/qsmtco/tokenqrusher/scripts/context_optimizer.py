...
# Maximum session summaries to keep (prevents unbounded growth)
...
def save_usage_state(state):
    ...
    if "session_summaries" in state and len(state["session_summaries"]) > MAX_SESSION_SUMMARIES:
        state["session_summaries"] = state["session_summaries"][-MAX_SESSION_SUMMARIES:]
